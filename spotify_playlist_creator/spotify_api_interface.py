        ...
        if len(playlist_id) == 0:
            try:
                spotify_obj().user_playlist_create('bdfgc5lgagjyvwig35m3uy834',
                                               f"Top 50 {artist_name} Songs Right Now", public=True,
                                               description=f"The most popular songs by {artist_name}! Updated Daily.")
            except SpotifyException:
                #Sometimes the fx returns a server error here, want the program to try again.
                pass
            continue
        break
    spotify_obj().user_playlist_replace_tracks('bdfgc5lgagjyvwig35m3uy834',playlist_id=playlist_id[0],
                                                 tracks=uris_list)
#SQL Interactions
